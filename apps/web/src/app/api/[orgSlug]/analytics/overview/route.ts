 ...
 *
 * That read fans out one STS AssumeRole plus one `GetMetricData` per connected
 * AWS account, inside the request, so the whole computation is wrapped in a
 * 5-minute `unstable_cache` tagged `analytics-overview-<orgId>` — the same
 * protection the sibling `/analytics/email-chart` route carries. Without it
 * every refetch re-ran the cross-account fan-out and the route's latency scaled
 * with the org's account count. `refreshAnalyticsCache` expires the tag so the
 * refresh button still recomputes on demand.
 */
function buildAnalyticsOverview(orgId: string, days: number) {
  return unstable_cache(
    ...
    ["analytics-overview", orgId, String(days)],
    { revalidate: 300, tags: [`analytics-overview-${orgId}`] }
  );
}
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const getCachedOverview = buildAnalyticsOverview(
