...
export function resolveConfig(config?: AWSWorldConfig): ResolvedConfig {
  ...
  const tablePrefix = config?.tablePrefix ?? process.env.WORKFLOW_AWS_TABLE_PREFIX ?? "workflow";
  const queuePrefix = config?.queuePrefix ?? process.env.WORKFLOW_AWS_QUEUE_PREFIX ?? "workflow";
  const endpoint = config?.endpoint ?? process.env.WORKFLOW_AWS_ENDPOINT;
  const deploymentId = config?.deploymentId ?? process.env.WORKFLOW_AWS_DEPLOYMENT_ID ?? `aws-${region}`;
