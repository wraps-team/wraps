...
export async function verify(options: VerifyOptions): Promise<void> {
  ...
  try {
    identity = await progress.execute('Checking SES verification status', async () => {
  ...
  const dnsResults: Array<{ name: string; type: string; status: string; records?: string[] }> = [];
  ...
  clack.note(
    [
      ...
      `${pc.bold('Verification Status:')} ${
        verificationStatus === 'verified' ? pc.green('✓ Verified') : pc.yellow('⏱ Pending')
      }`,
      `${pc.bold('DKIM Status:')} ${
        dkimStatus === 'SUCCESS' ? pc.green('✓ Success') : pc.yellow(`⏱ ${dkimStatus}`)
      }`,
    ].join('\n'),
    'SES Status'
  );
  ...
  if (verificationStatus === 'verified' && allVerified) {
    clack.outro(pc.green('✓ Domain is fully verified and ready to send emails!'));
  } else if (someIncorrect) {
    clack.outro(pc.red('✗ Some DNS records are incorrect. Please update them.'));
    console.log(`\nRun ${pc.cyan('byo status')} to see the correct DNS records.\n`);
  } else {
    clack.outro(pc.yellow('⏱ Waiting for DNS propagation and SES verification'));
    console.log(`\nDNS records can take up to 48 hours to propagate.`);
    console.log(`SES verification usually completes within 72 hours after DNS propagation.\n`);
  }
}
