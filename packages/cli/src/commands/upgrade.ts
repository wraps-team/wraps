...
import { UpgradeOptions, EmailStackConfig, BYOFeature } from '../types/index.js';
...
export async function upgrade(options: UpgradeOptions): Promise<void> {
  ...
  if (featureMap.configSet?.enabled) enabledFeatures.push('configSet');
  if (featureMap.bounceHandling?.enabled) enabledFeatures.push('bounceHandling');
  if (featureMap.complaintHandling?.enabled) enabledFeatures.push('complaintHandling');
  if (featureMap.emailHistory?.enabled) enabledFeatures.push('emailHistory');
  if (featureMap.eventProcessor?.enabled) enabledFeatures.push('eventProcessor');
  if (featureMap.dashboardAccess?.enabled) enabledFeatures.push('dashboardAccess');
  ...
  const featureConfigs: Record<string, FeatureConfig> = { ...metadata.features };
  ...
  const integrationLevel =
    allEnabledFeatures.includes('emailHistory') || allEnabledFeatures.includes('eventProcessor')
      ? 'enhanced'
      : 'dashboard-only';
    ...
    outputs = await progress.execute(
      ...
      async () => {
        ...
        const stack = await pulumi.automation.LocalWorkspace.createOrSelectStack(
          {
            stackName: metadata.pulumiStackName || `byo-${identity.accountId}-${region}`,
  ...
  if (replacedFeatures.length > 0) {
    ...
    for (const [feature, config] of replacedFeatures) {
      console.log(`  ${pc.cyan(config.originalValue!)} → ${pc.green(config.currentValue!)}`);
    }
