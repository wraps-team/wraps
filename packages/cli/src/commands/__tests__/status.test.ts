...
describe('status command', () => {
  ...
  it('should exit when no Pulumi stack is found', async () => {
    ...
    vi.mocked(LocalWorkspace.selectStack).mockRejectedValue(new Error('Stack not found'));
  ...
  it.skip('should display status with enhanced integration when configSet exists', async () => {
    ...
    const { LocalWorkspace } = await import('@pulumi/pulumi/automation');
    const mockStack = {
      outputs: async () => mockOutputs,
    };
    vi.mocked(LocalWorkspace.selectStack).mockResolvedValue(mockStack as any);
    ...
    expect(displayStatus).toHaveBeenCalledWith(
      expect.objectContaining({
        domains: expect.arrayContaining([
          expect.objectContaining({ domain: 'verified.com', status: 'verified' }),
