...
import { readFile, writeFile, unlink, mkdir, rmdir, readdir } from 'fs/promises';
...
describe('createConnectionMetadata', () => {
  it('should create metadata with required fields', () => {
    const metadata = createConnectionMetadata('123456789012', 'us-east-1', 'vercel');
  ...
  it('should generate ISO timestamp', () => {
    ...
    const metadata = createConnectionMetadata('123456789012', 'us-east-1', 'aws');
...
describe('updateFeatureMetadata', () => {
  ...
  it('should handle multiple features', () => {
    ...
    expect(metadata.features.emailHistory?.currentValue).toBe('byo-email-history');
  });
});
...
describe('getModifiedIdentities', () => {
  ...
  it('should return both attached and replaced identities', () => {
    ...
    expect(modified.map((i) => i.name).sort()).toEqual(['attached.com', 'replaced.com']);
  });
});
...
describe('deleteConnectionMetadata', () => {
  it('should delete file when it exists', async () => {
    ...
    expect(unlink).toHaveBeenCalledWith('/mock/byo/dir/connections/123456789012-us-east-1.json');
  });

  it('should not throw when file does not exist', async () => {
    vi.mocked(existsSync).mockReturnValue(false);

    await expect(deleteConnectionMetadata('123456789012', 'us-east-1')).resolves.toBeUndefined();
...
describe('listConnections', () => {
  ...
  it('should return all connection metadata files', async () => {
    ...
    const metadata1 = createConnectionMetadata('123456789012', 'us-east-1', 'vercel');
    const metadata2 = createConnectionMetadata('999888777666', 'eu-west-1', 'aws');
  ...
  it('should handle parse errors gracefully', async () => {
    ...
    const validMetadata = createConnectionMetadata('123456789012', 'us-east-1', 'vercel');
    vi.mocked(readFile)
...
describe('connectionExists', () => {
  it('should return true when file exists', async () => {
    ...
    expect(existsSync).toHaveBeenCalledWith('/mock/byo/dir/connections/123456789012-us-east-1.json');
  });
