...
describe('validateAWSCredentials', () => {
  ...
  it('should throw BYOError when credentials are invalid', async () => {
    ...
    await expect(validateAWSCredentials()).rejects.toThrow('AWS credentials not found');
  });
...
describe('listSESDomains', () => {
  ...
  it('should return empty array on API error', async () => {
    ...
    expect(domains).toEqual([]);
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error listing SES domains:', expect.any(Error));

    consoleErrorSpy.mockRestore();
  });

  it('should handle multiple domains correctly', async () => {
    const domainList = ['domain1.com', 'domain2.com', 'domain3.com', 'domain4.com'];
